	...
	public function send_sms($id){				
		...
		if ($this->request->is('post')) {
			...
			if(count($response_array)){
				...
				if(!strlen($result))
					$this->Session->setFlash(__("Not sent."));
				else
			...
			$this->Session->setFlash(__("Compose your message."));
		}
	...
	public function add($id=null) {
		if ($this->request->is('post')) {
			if(!isset($this->request->data['Contact']['contact_list_id']) || empty($this->request->data['Contact']['contact_list_id'])){
				$this->Session->setFlash(__('Select or create a contact list first and try again.'));
