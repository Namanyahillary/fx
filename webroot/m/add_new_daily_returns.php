...
<?php
	...
	if(	isset($_REQUEST['company_id']) && !empty($_REQUEST['company_id']) &&
		...
		$q=mysql_query("select id from daily_returns where id='".$daily_return_id."'");
		if(mysql_num_rows($q)){
			$result="Today's return Exists. Contact admin to edit it.";
		}else{
			@mysql_query("INSERT into daily_returns(id,fox_id,date,daily_buying_return_id,daily_selling_return_id,user_id,name) 
				values('$daily_return_id','$company_id','$date','$daily_return_id','$daily_return_id','$luid','$lun')")
				or die(mysql_error());		
			@mysql_query("INSERT into daily_buying_returns(id,fox_id,daily_return_id,c1,c2,c3,c4,c5,c6,c7,c8,date) 
				values('$daily_return_id','$company_id','$daily_return_id',".($_REQUEST['currency_rates_buying']).",'$date')")
				or die(mysql_error());
			@mysql_query("INSERT into daily_selling_returns(id,fox_id,daily_return_id,c1,c2,c3,c4,c5,c6,c7,c8,date) 
				values('$daily_return_id','$company_id','$daily_return_id',".($_REQUEST['currency_rates_selling']).",'$date')")
				or die(mysql_error());
			
			echo "Success saved locally!";exit;
		}
