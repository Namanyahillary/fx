...
<?php
	...
	if(	isset($_REQUEST['receipt_number']) && !empty($_REQUEST['receipt_number']) &&
		...
		@mysql_query(sprintf("INSERT into sold_receipts(id,fox_id,customer_name,amount,purpose_id,rate,amount_ugx,currency_id,instrument,date,nationality,address,passport_number,status,user_id,name,orig_amount,orig_rate,other_name) 
			values('$receipt_id','%d','%s','$amount','%s','$rate','$amount_ugx','%s','%s','$date','%s','%s','%s','$status','%s','%s','$orig_amount','$orig_rate','$other_currency_name')",
